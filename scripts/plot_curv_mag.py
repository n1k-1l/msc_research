...
recompute in `curvature_log` (see DropoutProbHook), producing:
  ...
  1. Correlation vs training: Pearson correlation rho between curvature and
     magnitude per hidden layer, mean +/- std over seeds, across every recompute
     checkpoint -- shows whether the relationship persists/strengthens through
     optimisation.
  2. Scatter plots: curvature vs magnitude per neuron at a few checkpoints
     (first / middle / last recompute), pooled over seeds, annotated with rho.
...
def correlation_figure(logs: list[list[dict]], out: Path, title: str) -> None:
    """Pearson rho(curv, mag) vs epoch, per layer, mean +/- std over seeds."""
    ...
    for L in range(n_layers):
        ...
        vals = np.array([[e["layers"][L].get("pearson_curv_mag", np.nan) for e in log]
                         for log in logs], dtype=float)
        mean = np.nanmean(vals, axis=0)
        std = np.nanstd(vals, axis=0)
        ax.plot(epochs, mean, marker="o", ms=3, color="C1", label=r"Pearson $\rho$")
        ax.fill_between(epochs, mean - std, mean + std, alpha=0.2, color="C1")
        ax.axhline(0, color="k", lw=0.5)
        ax.set_ylim(-1.05, 1.05)
        ax.set_xlabel("epoch")
        ax.set_ylabel(r"$\rho$(curvature, magnitude)")
...
def scatter_figure(logs: list[list[dict]], out: Path, title: str) -> None:
    ...
    for r, ci in enumerate(ckpts):
        ...
        for L in range(n_layers):
            ...
            rho = float(np.nanmean([log[ci]["layers"][L].get("pearson_curv_mag", np.nan)
                                    for log in logs]))
            ax.set_title(f"layer {L}, epoch {epoch}  (rho={rho:.2f})")
